...
class ChartImageService
{
    ...
    /**
     * Horizontal bar chart (e.g. cumulative impact) — matches dashboard-style vertical BarChart layout.
     *
     * @param  array{
     *     labels: list<string>,
     *     data: list<float|int>,
     ...
     * }  $config
     */
    public function generateHorizontalBarChart(array $config, string $filename): ?string
    {
        $data = $config['data'] ?? [];
        $labels = $config['labels'] ?? [];
        $colors = $config['colors'] ?? $this->getDefaultColors(max(1, count($data)));
        if (count($colors) < count($data)) {
            $extra = $this->getDefaultColors(count($data) - count($colors));
            $colors = array_merge($colors, $extra);
        }
        ...
        $width = $config['width'] ?? self::DEFAULT_WIDTH;
        $height = $config['height'] ?? 280;

        return $this->generateChart($chartConfig, $filename, $width, $height);
    }
