...
export default function SheqComplianceIndex({ documents }) {
    ...
    const handleDelete = (document) => {
        if (confirm(`Delete document "${document.title}"?`)) {
            router.delete(`/sheq-compliance/${document.id}`, { preserveScroll: true });
        }
    };

    const moveUp = (document) => {
        router.post(`/sheq-compliance/${document.id}/move-up`, {}, { preserveScroll: true });
    };

    const moveDown = (document) => {
        router.post(`/sheq-compliance/${document.id}/move-down`, {}, { preserveScroll: true });
    };
    ...
    const handleSubmit = (event) => {
        ...
        if (mode === 'create') {
            ...
            form.post(`/sheq-compliance/${editingId}`, {
                forceFormData: true,
                preserveScroll: true,
                onSuccess: closeModal,
            });
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <div className="flex items-center space-x-3">
                    <a
                        href={`/sheq-compliance/${row.original.id}/view`}
                        target="_blank"
                    ...
                    <a
                        href={`/sheq-compliance/${row.original.id}/download`}
                        className="text-primary-600 hover:text-primary-800 dark:text-primary-400"
                        title="Download"
                    >
                    ...
                    {canManage && (
                        <>
                            <button onClick={() => moveUp(row.original)} className="text-gray-500 hover:text-gray-800 dark:hover:text-gray-200" title="Move up">
                                <ArrowUp className="h-4 w-4" />
                            </button>
                            <button onClick={() => moveDown(row.original)} className="text-gray-500 hover:text-gray-800 dark:hover:text-gray-200" title="Move down">
    ...
    return (
        <DashboardLayout title="SHEQ Compliance">
            <Head title="SHEQ Compliance" />

            <div className="mb-6 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                <div>
                    <h1 className="text-2xl font-semibold text-gray-900 dark:text-gray-100">SHEQ Compliance</h1>
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                        Safety, Health, Environment &amp; Quality (HSE File) documents uploaded by WasteFlow staff.
                    </p>
                </div>
                {canManage && (
                    <button
            ...
            <DataTable data={documents.data || []} columns={columns} title="All SHEQ Compliance Documents" pagination={false} />
                ...
                <Modal show={modalOpen} onClose={closeModal} maxWidth="lg">
                    <form onSubmit={handleSubmit} className="p-6 space-y-6">
                        ...
                        <div className="space-y-4">
                            <div>
                                <InputLabel htmlFor="sheq-document-title" value="Title" />
                                <TextInput
                                    id="sheq-document-title"
                            ...
                            <div>
                                <InputLabel htmlFor="sheq-document-description" value="Description" />
                                <textarea
                                    id="sheq-document-description"
                            ...
                            <div>
                                <InputLabel htmlFor="sheq-document-file" value={mode === 'create' ? 'File' : 'Replace file (optional)'} />
                                <input
                                    ref={fileInputRef}
                                    id="sheq-document-file"
                                    type="file"
