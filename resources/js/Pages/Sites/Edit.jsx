...
export default function Edit({ site, companies }) {
    // Determine the initial company - either direct or through branch
    const getInitialCompany = () => {
        if (site.company_id) {
            return site.company_id.toString();
        }
        // Find company through branch
        if (site.branch_id) {
            for (const company of companies) {
                if (company.branches?.some(b => b.id === site.branch_id)) {
    ...
    const [selectedCompany, setSelectedCompany] = useState(getInitialCompany());
    const [useDirectCompany, setUseDirectCompany] = useState(!!site.company_id && !site.branch_id);
    ...
    // Initialize selected company when component loads (already handled in useState initializer)
    ...
    return (
        <DashboardLayout title="Edit Site">
